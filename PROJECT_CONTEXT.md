...
Use CSS variables for color-theme-sensitive styling.
Do not reintroduce unused dice SVG/icon assets.
Keep Foundry VTT v13 compatibility.
Prefer extending existing Foundry systems instead of replacing them.
Keep the archive inner folder named exactly `imagine-interface`.
...
Update the module version in `module.json`.
Keep the repository structure intact.
Keep the inner archive folder named `imagine-interface`.
Package the release as `imagine-interface-vX.X.X.zip`.
Also provide a standalone `module.json` for the GitHub release.
Ensure `assets` does not contain removed dice/icon files unless explicitly requested.
Keep styling consistent across all color schemes.
Test JavaScript syntax if possible before returning files.
